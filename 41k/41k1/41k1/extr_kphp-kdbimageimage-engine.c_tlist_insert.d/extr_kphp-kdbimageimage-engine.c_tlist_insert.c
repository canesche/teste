...
struct forth_request *_allocate_L(int length, struct forth_request *aux_L[]) {
...
walker->size = ((-2 * (next_i()%2)) + 1) * next_i();
  ...
  for(int i = 1; i < length; i++) {
    walker->next = (struct forth_request *)malloc(sizeof(struct forth_request));
    walker = walker->next;
    aux_L[i] = walker;
...
void _delete_L(struct forth_request *aux_L[], int aux_L_size) {
  for(int i = 0; i < aux_L_size; i++) 
    if(aux_L[i])
      free(aux_L[i]);
}

struct forth_request *_allocate_T(int length, struct forth_request *aux_T[]) {
  struct forth_request *walker = (struct forth_request *)malloc(sizeof(struct forth_request));
...
void _delete_T(struct forth_request *aux_T[], int aux_T_size) {
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct forth_request * aux_L[1];
          struct forth_request * L = _allocate_L(1, aux_L);
          struct forth_request * aux_T[1];
          struct forth_request * T = _allocate_T(1, aux_T);
          tlist_insert(L,T);
          _delete_L(aux_L, 1);
          _delete_T(aux_T, 1);
    ...
    case 3:
    {
          struct forth_request * aux_L[10000];
          struct forth_request * L = _allocate_L(10000, aux_L);
          struct forth_request * aux_T[10000];
          struct forth_request * T = _allocate_T(10000, aux_T);
          tlist_insert(L,T);
