...
__attribute__((used)) static void draw_sample_point_rgba_scale(uint8_t *buf, int height, int linesize,
                                         int16_t *prev_y,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_color0 = 4;
          long const* color = (long const*) malloc(_len_color0*sizeof(long const));
          for(int _i0 = 0; _i0 < _len_color0; _i0++) {
            color[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          draw_sample_point_rgba_scale(buf,height,linesize,prev_y,color,h);
          free(buf);
          free(prev_y);
          free(color);
