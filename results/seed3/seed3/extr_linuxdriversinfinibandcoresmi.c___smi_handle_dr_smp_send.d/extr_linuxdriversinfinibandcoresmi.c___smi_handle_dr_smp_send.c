...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          const unsigned long * return_path = (const unsigned long *) malloc(_len_return_path0*sizeof(const unsigned long));
          for(int _i0 = 0; _i0 < _len_return_path0; _i0++) {
            return_path[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          enum smi_action benchRet = __smi_handle_dr_smp_send(is_switch,port_num,hop_ptr,hop_cnt,initial_path,return_path,direction,dr_dlid_is_permissive,dr_slid_is_permissive);
          free(hop_ptr);
          free(initial_path);
          free(return_path);
