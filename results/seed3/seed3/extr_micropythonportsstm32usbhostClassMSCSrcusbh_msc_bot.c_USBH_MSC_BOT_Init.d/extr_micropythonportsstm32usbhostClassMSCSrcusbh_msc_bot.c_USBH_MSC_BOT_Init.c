...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_phost__i0__pActiveClass_pData0; _j0++) {
            phost[_i0].pActiveClass->pData->hbot.cmd_state = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->hbot.state = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->hbot.cbw.field.Tag = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->hbot.cbw.field.Signature = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          int benchRet = USBH_MSC_BOT_Init(phost);
          printf("%d\n", benchRet); 
