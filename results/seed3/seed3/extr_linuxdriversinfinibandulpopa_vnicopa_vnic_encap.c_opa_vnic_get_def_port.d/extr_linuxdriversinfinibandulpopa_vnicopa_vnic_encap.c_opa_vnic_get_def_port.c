// ========================================================================= //

// includes
#include "stdio.h"
#include "stdlib.h"
#include "time.h"
#include "string.h"
#include "limits.h"
#include "float.h"



#define JOTAI_NUM_RANDS_ 25

const unsigned rand_primes[JOTAI_NUM_RANDS_] = {179, 103, 479, 647, 229, 37, 271, 557, 263, 607, 18743, 50359, 21929, 48757, 98179, 12907, 52937, 64579, 49957, 52567, 507163, 149939, 412157, 680861, 757751};

int next_i() {
  int counter = 0;
  return rand_primes[(++counter)%JOTAI_NUM_RANDS_];
}

float next_f() {
  int counter = 0;
  return rand_primes[(++counter)%JOTAI_NUM_RANDS_] / 757751.0F;
} 


// Usage menu
void usage() {
    printf("%s", "Usage:\n\
    prog [ARGS]\n\
\nARGS:\n\
       0            int-bounds\n\
       1            big-arr\n\
       2            big-arr-10x\n\
\n\
");

}


// ------------------------------------------------------------------------- //

#define NULL ((void*)0)
typedef unsigned long size_t;  // Customize by platform.
typedef long intptr_t; typedef unsigned long uintptr_t;
typedef long scalar_t__;  // Either arithmetic or pointer type.
/* By default, we understand bool (as a convenience). */
typedef int bool;
#define false 0
#define true 1

/* Forward declarations */

/* Type definitions */
typedef  int u8 ;
struct opa_vnic_adapter {int* flow_tbl; } ;

/* Variables and functions */
 int OPA_VNIC_FLOW_TBL_SIZE ; 

__attribute__((used)) static inline u8 opa_vnic_get_def_port(struct opa_vnic_adapter *adapter,
				       u8 entropy)
{
	u8 flow_id;

	/* Add the upper and lower 4-bits of entropy to get the flow id */
	flow_id = ((entropy & 0xf) + (entropy >> 4));
	return adapter->flow_tbl[flow_id & (OPA_VNIC_FLOW_TBL_SIZE - 1)];
}


// ------------------------------------------------------------------------- //

int main(int argc, char *argv[]) {

    if (argc != 2) {
        usage();
        return 1;
    }

    int opt = atoi(argv[1]);
    switch(opt) {

    // int-bounds
    case 0:
    {
          int entropy = 100;
          int _len_adapter0 = 1;
          struct opa_vnic_adapter * adapter = (struct opa_vnic_adapter *) malloc(_len_adapter0*sizeof(struct opa_vnic_adapter));
          for(int _i0 = 0; _i0 < _len_adapter0; _i0++) {
              int _len_adapter__i0__flow_tbl0 = 1;
          adapter[_i0].flow_tbl = (int *) malloc(_len_adapter__i0__flow_tbl0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_adapter__i0__flow_tbl0; _j0++) {
            adapter[_i0].flow_tbl[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = opa_vnic_get_def_port(adapter,entropy);
          printf("%d\n", benchRet); 
          for(int _aux = 0; _aux < _len_adapter0; _aux++) {
          free(adapter[_aux].flow_tbl);
          }
          free(adapter);
        
        break;
    }
    // big-arr
    case 1:
    {
          int entropy = 255;
          int _len_adapter0 = 65025;
          struct opa_vnic_adapter * adapter = (struct opa_vnic_adapter *) malloc(_len_adapter0*sizeof(struct opa_vnic_adapter));
          for(int _i0 = 0; _i0 < _len_adapter0; _i0++) {
              int _len_adapter__i0__flow_tbl0 = 1;
          adapter[_i0].flow_tbl = (int *) malloc(_len_adapter__i0__flow_tbl0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_adapter__i0__flow_tbl0; _j0++) {
            adapter[_i0].flow_tbl[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = opa_vnic_get_def_port(adapter,entropy);
          printf("%d\n", benchRet); 
          for(int _aux = 0; _aux < _len_adapter0; _aux++) {
          free(adapter[_aux].flow_tbl);
          }
          free(adapter);
        
        break;
    }
    // big-arr-10x
    case 2:
    {
          int entropy = 10;
          int _len_adapter0 = 100;
          struct opa_vnic_adapter * adapter = (struct opa_vnic_adapter *) malloc(_len_adapter0*sizeof(struct opa_vnic_adapter));
          for(int _i0 = 0; _i0 < _len_adapter0; _i0++) {
              int _len_adapter__i0__flow_tbl0 = 1;
          adapter[_i0].flow_tbl = (int *) malloc(_len_adapter__i0__flow_tbl0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_adapter__i0__flow_tbl0; _j0++) {
            adapter[_i0].flow_tbl[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = opa_vnic_get_def_port(adapter,entropy);
          printf("%d\n", benchRet); 
          for(int _aux = 0; _aux < _len_adapter0; _aux++) {
          free(adapter[_aux].flow_tbl);
          }
          free(adapter);
        
        break;
    }

    default:
        usage();
        break;

    }

    return 0;
}
