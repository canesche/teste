// ========================================================================= //

// includes
#include "stdio.h"
#include "stdlib.h"
#include "time.h"
#include "string.h"
#include "limits.h"
#include "float.h"



#define JOTAI_NUM_RANDS_ 25

const unsigned rand_primes[JOTAI_NUM_RANDS_] = {179, 103, 479, 647, 229, 37, 271, 557, 263, 607, 18743, 50359, 21929, 48757, 98179, 12907, 52937, 64579, 49957, 52567, 507163, 149939, 412157, 680861, 757751};

int next_i() {
  int counter = 0;
  return rand_primes[(++counter)%JOTAI_NUM_RANDS_];
}

float next_f() {
  int counter = 0;
  return rand_primes[(++counter)%JOTAI_NUM_RANDS_] / 757751.0F;
} 


// Usage menu
void usage() {
    printf("%s", "Usage:\n\
    prog [ARGS]\n\
\nARGS:\n\
       0            int-bounds\n\
       1            big-arr\n\
       2            big-arr-10x\n\
\n\
");

}


// ------------------------------------------------------------------------- //

#define NULL ((void*)0)
typedef unsigned long size_t;  // Customize by platform.
typedef long intptr_t; typedef unsigned long uintptr_t;
typedef long scalar_t__;  // Either arithmetic or pointer type.
/* By default, we understand bool (as a convenience). */
typedef int bool;
#define false 0
#define true 1

/* Forward declarations */
typedef  struct TYPE_3__   TYPE_1__ ;

/* Type definitions */
typedef  int /*<<< orphan*/  token_type_t ;
struct TYPE_3__ {size_t pos; size_t len; size_t line; size_t col; int /*<<< orphan*/  token_type; int /*<<< orphan*/ * parser; } ;
typedef  TYPE_1__ token_t ;
typedef  int /*<<< orphan*/  parser_t ;

/* Variables and functions */

__attribute__((used)) static void
token_init(token_t *token, parser_t *parser, token_type_t token_type,
    size_t pos, size_t len, size_t line, size_t col) {
	token->parser = parser;
	token->token_type = token_type;
	token->pos = pos;
	token->len = len;
	token->line = line;
	token->col = col;
}


// ------------------------------------------------------------------------- //

int main(int argc, char *argv[]) {

    if (argc != 2) {
        usage();
        return 1;
    }

    int opt = atoi(argv[1]);
    switch(opt) {

    // int-bounds
    case 0:
    {
          int token_type = 100;
          unsigned long pos = 100;
          unsigned long len = 100;
          unsigned long line = 100;
          unsigned long col = 100;
          int _len_token0 = 1;
          struct TYPE_3__ * token = (struct TYPE_3__ *) malloc(_len_token0*sizeof(struct TYPE_3__));
          for(int _i0 = 0; _i0 < _len_token0; _i0++) {
            token[_i0].pos = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].len = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].line = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].col = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].token_type = ((-2 * (next_i()%2)) + 1) * next_i();
          int _len_token__i0__parser0 = 1;
          token[_i0].parser = (int *) malloc(_len_token__i0__parser0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_token__i0__parser0; _j0++) {
            token[_i0].parser[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int _len_parser0 = 1;
          int * parser = (int *) malloc(_len_parser0*sizeof(int));
          for(int _i0 = 0; _i0 < _len_parser0; _i0++) {
            parser[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          token_init(token,parser,token_type,pos,len,line,col);
          for(int _aux = 0; _aux < _len_token0; _aux++) {
          free(token[_aux].parser);
          }
          free(token);
          free(parser);
        
        break;
    }
    // big-arr
    case 1:
    {
          int token_type = 255;
          unsigned long pos = 255;
          unsigned long len = 255;
          unsigned long line = 255;
          unsigned long col = 255;
          int _len_token0 = 65025;
          struct TYPE_3__ * token = (struct TYPE_3__ *) malloc(_len_token0*sizeof(struct TYPE_3__));
          for(int _i0 = 0; _i0 < _len_token0; _i0++) {
            token[_i0].pos = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].len = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].line = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].col = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].token_type = ((-2 * (next_i()%2)) + 1) * next_i();
          int _len_token__i0__parser0 = 1;
          token[_i0].parser = (int *) malloc(_len_token__i0__parser0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_token__i0__parser0; _j0++) {
            token[_i0].parser[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int _len_parser0 = 65025;
          int * parser = (int *) malloc(_len_parser0*sizeof(int));
          for(int _i0 = 0; _i0 < _len_parser0; _i0++) {
            parser[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          token_init(token,parser,token_type,pos,len,line,col);
          for(int _aux = 0; _aux < _len_token0; _aux++) {
          free(token[_aux].parser);
          }
          free(token);
          free(parser);
        
        break;
    }
    // big-arr-10x
    case 2:
    {
          int token_type = 10;
          unsigned long pos = 10;
          unsigned long len = 10;
          unsigned long line = 10;
          unsigned long col = 10;
          int _len_token0 = 100;
          struct TYPE_3__ * token = (struct TYPE_3__ *) malloc(_len_token0*sizeof(struct TYPE_3__));
          for(int _i0 = 0; _i0 < _len_token0; _i0++) {
            token[_i0].pos = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].len = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].line = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].col = ((-2 * (next_i()%2)) + 1) * next_i();
        token[_i0].token_type = ((-2 * (next_i()%2)) + 1) * next_i();
          int _len_token__i0__parser0 = 1;
          token[_i0].parser = (int *) malloc(_len_token__i0__parser0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_token__i0__parser0; _j0++) {
            token[_i0].parser[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int _len_parser0 = 100;
          int * parser = (int *) malloc(_len_parser0*sizeof(int));
          for(int _i0 = 0; _i0 < _len_parser0; _i0++) {
            parser[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          token_init(token,parser,token_type,pos,len,line,col);
          for(int _aux = 0; _aux < _len_token0; _aux++) {
          free(token[_aux].parser);
          }
          free(token);
          free(parser);
        
        break;
    }

    default:
        usage();
        break;

    }

    return 0;
}
