...
__attribute__((used)) static int nvdec_mjpeg_decode_slice(AVCodecContext *avctx, const uint8_t *buffer, uint32_t size)
{
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int benchRet = nvdec_mjpeg_decode_slice(avctx,buffer,size);
          printf("%d\n", benchRet); 
          free(avctx);
          free(buffer);
