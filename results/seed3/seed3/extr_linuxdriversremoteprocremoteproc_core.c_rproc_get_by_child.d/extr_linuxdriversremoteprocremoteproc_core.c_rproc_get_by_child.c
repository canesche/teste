...
struct device *_allocate_dev(int length, struct device *aux_dev[]) {
  ...
  aux_dev[0] = walker;
  int _len_walker_driver_data0 = 1;
  walker->driver_data = (struct rproc *) malloc(_len_walker_driver_data0*sizeof(struct rproc));
  for(int _i0 = 0; _i0 < _len_walker_driver_data0; _i0++) {
    walker->driver_data.dummy = ((-2 * (next_i()%2)) + 1) * next_i();
  }
  int _len_walker_type0 = 1;
  walker->type = (int *) malloc(_len_walker_type0*sizeof(int));
  for(int _i0 = 0; _i0 < _len_walker_type0; _i0++) {
    walker->type[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
  }
  walker->parent = NULL;
...
int main(int argc, char *argv[]) {
    ...
    case 3:
    {
          struct device * aux_dev[10000];
          struct device * dev = _allocate_dev(10000, aux_dev);
          struct rproc * benchRet = rproc_get_by_child(dev);
          printf("%d\n", (*benchRet).dummy);
