...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_info0 = 100;
          struct gasket_cdev_info * info = (struct gasket_cdev_info *) malloc(_len_info0*sizeof(struct gasket_cdev_info));
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info[_i0].ownership.owner = ((-2 * (next_i()%2)) + 1) * next_i();
        info[_i0].ownership.is_owned = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = gasket_owned_by_current_tgid(info);
          printf("%d\n", benchRet); 
          free(info);
