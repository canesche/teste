...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          rdev[_i0].pm.dpm.priv = (struct igp_power_info *) malloc(_len_rdev__i0__pm_dpm_priv0*sizeof(struct igp_power_info));
          for(int _j0 = 0; _j0 < _len_rdev__i0__pm_dpm_priv0; _j0++) {
            rdev[_i0].pm.dpm.priv->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct igp_power_info * benchRet = rs780_get_pi(rdev);
          printf("%d\n", (*benchRet).dummy);
          free(rdev);
