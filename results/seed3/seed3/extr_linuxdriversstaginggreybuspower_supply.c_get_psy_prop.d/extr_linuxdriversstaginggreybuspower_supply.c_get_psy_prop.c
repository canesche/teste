...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_gbpsy__i0__props0 = 1;
          gbpsy[_i0].props = (struct gb_power_supply_prop *) malloc(_len_gbpsy__i0__props0*sizeof(struct gb_power_supply_prop));
          for(int _j0 = 0; _j0 < _len_gbpsy__i0__props0; _j0++) {
            gbpsy[_i0].props->prop = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct gb_power_supply_prop * benchRet = get_psy_prop(gbpsy,psp);
          printf("%d\n", (*benchRet).prop);
          for(int _aux = 0; _aux < _len_gbpsy0; _aux++) {
          free(gbpsy[_aux].props);
          }
          free(gbpsy);
