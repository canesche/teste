    ...
    case 0:
    {
        ...
        phost[_i0].device.CfgDesc.Itf_Desc->bInterfaceNumber = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          for(int _j0 = 0; _j0 < _len_phost__i0__pActiveClass_pData0; _j0++) {
              int _len_phost__i0__pActiveClass_pData_stream_in0 = 1;
          phost[_i0].pActiveClass->pData->stream_in = (struct TYPE_15__ *) malloc(_len_phost__i0__pActiveClass_pData_stream_in0*sizeof(struct TYPE_15__));
          for(int _j0 = 0; _j0 < _len_phost__i0__pActiveClass_pData_stream_in0; _j0++) {
            phost[_i0].pActiveClass->pData->stream_in->Ep = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->stream_in->EpSize = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->stream_in->valid = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->stream_in->Poll = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->stream_in->AltSettings = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->stream_in->interface = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          }
          int benchRet = USBH_AUDIO_FindAudioStreamingIN(phost);
          printf("%d\n", benchRet); 
