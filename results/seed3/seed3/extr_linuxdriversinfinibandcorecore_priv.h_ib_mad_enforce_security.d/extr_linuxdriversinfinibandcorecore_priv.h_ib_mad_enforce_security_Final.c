...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          int pkey_index = 100;
          int _len_map0 = 1;
          struct ib_mad_agent_private * map = (struct ib_mad_agent_private *) malloc(_len_map0*sizeof(struct ib_mad_agent_private));
          for(int _i0 = 0; _i0 < _len_map0; _i0++) {
            map[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = ib_mad_enforce_security(map,pkey_index);
          printf("%d\n", benchRet); 
          free(map);
