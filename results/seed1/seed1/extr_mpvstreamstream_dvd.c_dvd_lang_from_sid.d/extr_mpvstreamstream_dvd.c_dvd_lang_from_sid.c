...
__attribute__((used)) static int dvd_lang_from_sid(stream_t *stream, int id) {
  int i;
  dvd_priv_t *d;
  if (!stream) return 0;
  d = stream->priv;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_stream__i0__priv0; _j0++) {
            stream[_i0].priv->nr_of_subtitles = ((-2 * (next_i()%2)) + 1) * next_i();
          int _len_stream__i0__priv_subtitles0 = 1;
          stream[_i0].priv->subtitles = (struct TYPE_5__ *) malloc(_len_stream__i0__priv_subtitles0*sizeof(struct TYPE_5__));
          for(int _j0 = 0; _j0 < _len_stream__i0__priv_subtitles0; _j0++) {
            stream[_i0].priv->subtitles->id = ((-2 * (next_i()%2)) + 1) * next_i();
        stream[_i0].priv->subtitles->language = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          int benchRet = dvd_lang_from_sid(stream,id);
          printf("%d\n", benchRet); 
