...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_ins0; _i0++) {
            ins[_i0].declaration.indexable_temp.component_count = ((-2 * (next_i()%2)) + 1) * next_i();
        ins[_i0].declaration.indexable_temp.register_size = ((-2 * (next_i()%2)) + 1) * next_i();
        ins[_i0].declaration.indexable_temp.register_idx = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_tokens0 = 1;
          ...
          shader_sm4_read_dcl_indexable_temp(ins,opcode,opcode_token,tokens,token_count,priv);
          free(ins);
          free(tokens);
          free(priv);
