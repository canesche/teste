...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          p[_i0].physport->private_data = (struct parport_ip32_private *) malloc(_len_p__i0__physport_private_data0*sizeof(struct parport_ip32_private));
          for(int _j0 = 0; _j0 < _len_p__i0__physport_private_data0; _j0++) {
            p[_i0].physport->private_data->dcr_cache = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          unsigned int benchRet = __parport_ip32_read_control(p);
          printf("%u\n", benchRet); 
