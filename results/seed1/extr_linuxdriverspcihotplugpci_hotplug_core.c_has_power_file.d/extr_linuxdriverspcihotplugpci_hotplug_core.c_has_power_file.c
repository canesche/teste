...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_pci_slot__i0__hotplug_ops0; _j0++) {
            pci_slot[_i0].hotplug->ops->get_power_status = ((-2 * (next_i()%2)) + 1) * next_i();
        pci_slot[_i0].hotplug->ops->disable_slot = ((-2 * (next_i()%2)) + 1) * next_i();
        pci_slot[_i0].hotplug->ops->enable_slot = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          int benchRet = has_power_file(pci_slot);
          printf("%d\n", benchRet); 
