...
__attribute__((used)) static size_t php_stream_input_write(php_stream *stream, const char *buf, size_t count) /* {{{ */
{
	return -1;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          unsigned long benchRet = php_stream_input_write(stream,buf,count);
          printf("%lu\n", benchRet); 
          free(stream);
          free(buf);
