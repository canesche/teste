...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_gspca_dev__i0__cam_cam_mode0; _j0++) {
            gspca_dev[_i0].cam.cam_mode->width = ((-2 * (next_i()%2)) + 1) * next_i();
        gspca_dev[_i0].cam.cam_mode->height = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = wxh_to_nearest_mode(gspca_dev,width,height);
          printf("%d\n", benchRet); 
          free(gspca_dev);
