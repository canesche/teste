...
__attribute__((used)) static int vc4_dsi_host_detach(struct mipi_dsi_host *host,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_device0 = 1;
          struct mipi_dsi_device * device = (struct mipi_dsi_device *) malloc(_len_device0*sizeof(struct mipi_dsi_device));
          for(int _i0 = 0; _i0 < _len_device0; _i0++) {
            device[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = vc4_dsi_host_detach(host,device);
          printf("%d\n", benchRet); 
          free(host);
          free(device);
