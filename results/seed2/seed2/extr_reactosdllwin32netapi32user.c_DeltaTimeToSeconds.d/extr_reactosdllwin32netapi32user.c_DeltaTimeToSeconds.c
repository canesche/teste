...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct TYPE_3__ DeltaTime;
        DeltaTime.QuadPart = ((-2 * (next_i()%2)) + 1) * next_i();
        DeltaTime.HighPart = ((-2 * (next_i()%2)) + 1) * next_i();
        DeltaTime.LowPart = ((-2 * (next_i()%2)) + 1) * next_i();
          int benchRet = DeltaTimeToSeconds(DeltaTime);
          printf("%d\n", benchRet); 
