...
__attribute__((used)) static int keyring_find_key (const char *name, int name_len, unsigned char **key) {
  return -1;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_key0; _i0++) {
            int _len_key1 = 1;
            key[_i0] = (unsigned char *) malloc(_len_key1*sizeof(unsigned char));
            for(int _i1 = 0; _i1 < _len_key1; _i1++) {
              key[_i0][_i1] = ((-2 * (next_i()%2)) + 1) * next_i();
            }
          }
          int benchRet = keyring_find_key(name,name_len,key);
          printf("%d\n", benchRet); 
          free(name);
