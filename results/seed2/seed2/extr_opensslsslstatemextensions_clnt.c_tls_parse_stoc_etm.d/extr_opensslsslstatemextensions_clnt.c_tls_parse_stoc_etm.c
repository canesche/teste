    ...
    case 0:
    {
        ...
        s[_i0].ext.use_etm = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          for(int _j0 = 0; _j0 < _len_s__i0__s3_tmp_new_cipher0; _j0++) {
            s[_i0].s3.tmp.new_cipher->algorithm_mac = ((-2 * (next_i()%2)) + 1) * next_i();
        s[_i0].s3.tmp.new_cipher->algorithm_enc = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int _len_pkt0 = 1;
          ...
          int benchRet = tls_parse_stoc_etm(s,pkt,context,x,chainidx);
          printf("%d\n", benchRet); 
          free(s);
          free(pkt);
          free(x);
