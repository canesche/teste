...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_ssa__i0__ops0; _j0++) {
            ssa[_i0].ops->op1_use = ((-2 * (next_i()%2)) + 1) * next_i();
        ssa[_i0].ops->op2_use = ((-2 * (next_i()%2)) + 1) * next_i();
        ssa[_i0].ops->result_use = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = is_used_by_op(ssa,op,check);
          printf("%d\n", benchRet); 
