...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_mask0 = 1;
          const char * mask = (const char *) malloc(_len_mask0*sizeof(const char));
          for(int _i0 = 0; _i0 < _len_mask0; _i0++) {
            mask[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = mlxsw_sp1_acl_ctcam_region_entry_insert(cregion,centry,mask);
          printf("%d\n", benchRet); 
          free(cregion);
          free(centry);
          free(mask);
