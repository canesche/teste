...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_phost__i0__pActiveClass_pData0; _j0++) {
            phost[_i0].pActiveClass->pData->ptp.req_state = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->ptp.state = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          int benchRet = USBH_PTP_Init(phost);
          printf("%d\n", benchRet); 
