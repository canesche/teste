...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        phost[_i0].pActiveClass->pData->processing_state = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->control_state = ((-2 * (next_i()%2)) + 1) * next_i();
        phost[_i0].pActiveClass->pData->headphone.total_length = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          int benchRet = USBH_AUDIO_Play(phost,buf,length);
          printf("%d\n", benchRet); 
