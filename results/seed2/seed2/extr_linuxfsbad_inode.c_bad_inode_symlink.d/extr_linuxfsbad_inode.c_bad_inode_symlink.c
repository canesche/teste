...
__attribute__((used)) static int bad_inode_symlink (struct inode *dir, struct dentry *dentry,
		const char *symname)
{
	return -EIO;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_symname0 = 1;
          const char * symname = (const char *) malloc(_len_symname0*sizeof(const char));
          for(int _i0 = 0; _i0 < _len_symname0; _i0++) {
            symname[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = bad_inode_symlink(dir,dentry,symname);
          printf("%d\n", benchRet); 
          free(dir);
          free(dentry);
          free(symname);
