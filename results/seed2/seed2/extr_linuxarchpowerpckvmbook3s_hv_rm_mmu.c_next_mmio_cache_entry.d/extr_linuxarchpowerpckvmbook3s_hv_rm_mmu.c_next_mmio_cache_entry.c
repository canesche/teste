...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_vcpu0; _i0++) {
            vcpu[_i0].arch.mmio_cache.index = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          for(int _j0 = 0; _j0 < _len_vcpu__i0__arch_mmio_cache_entry0; _j0++) {
            vcpu[_i0].arch.mmio_cache.entry->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct mmio_hpte_cache_entry * benchRet = next_mmio_cache_entry(vcpu);
          printf("%d\n", (*benchRet).dummy);
          free(vcpu);
