...
for ( i = 0; i < 64; i++)
for ( j = 1; j < 64; j++)
for ( k = pips_max_4((-i-2*j-2)/3, -2*i-5*j-1, -i-j-1, 0); k < pips_min_4((-i-2*j+62)/3, -i-j+63, -2*i-5*j+63, 63); k++)
{
A[i][j][k]=A[i][j-1][k]-B[i+j+k+1][i+j+k+1][i+j+k+1]-C[i+2*j+3*k+4][2*i+5*j+k+1][k+1]+63;
}
